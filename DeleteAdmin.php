...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	...
	<table align="center" border="0">
        ...
        $rawres = mysqli_query($con,"SELECT * FROM person
            WHERE ((`Name` LIKE '%".$query."%') OR (`U_ID` LIKE '%".$query."%')) AND `Usertype`='Admin'") or die(mysqli_error($con));
        ...
        if(mysqli_num_rows($rawres) > 0){
			...
			<a href="DeleteAdminConfirm.php?uuid=<?php echo $dnn['U_ID']?>"><b><i> Delete Admin</i> </b></a>
			</td>
		...
		$query = 'select * from person where Usertype="Admin" and U_ID!="s" ';
